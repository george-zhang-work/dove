 ...
 * is guarantee to run inside {@link Activity} lifecycle. However, it can be dropped if the
...
public class UiHandler {
    ...
    public void setEnabled(boolean enabled) {
        mEnabled = enabled;
        if (!mEnabled) {
            final int count = mCount.getAndSet(0);
            if (count > 0) {
                LogUtils.d(LOG_TAG, "Disable UiHandler. Dropping  %d Runnables.", count);
            }
