...
//    @VisibleForTesting
    ...
    public static String contentUriToString(String tag, Uri uri) {
        if (isDebugLoggingEnabled(tag)) {
            ...
            // Debug logging is not enabled, we want to remove the email address
            // from the uri.
            List<String> pathSegments = uri.getPathSegments();

            Uri.Builder builder = new Uri.Builder().scheme(uri.getScheme())
                    .authority(uri.getAuthority()).query(uri.getQuery())
    ...
    /**
     * Sanitizes an account name. If debug logging is not enabled, a sanitized
     * name is returned.
     */
